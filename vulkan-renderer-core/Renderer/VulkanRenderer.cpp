...
bool VulkanRenderer::initialize(HINSTANCE hInstance, HWND hWnd, uint32_t width, uint32_t height)
{
	...
	*/
	{
		create_descriptor_set_and_pipeline_layout();
	}
	...
	*/
	{
		create_descriptor_pool(device, &descriptor_pool);
	...
	*/
	{
		...
		VK_CHECK_RESULT(vkCreatePipelineCache(device, &pipeline_cache_create_info, nullptr, &graphics_pipeline_cache));
	}
	...
	*/
	{
		...
		VK_CHECK_RESULT(vkCreateGraphicsPipelines(device, graphics_pipeline_cache, 1, &pipeline_create_info, nullptr, &graphics_pipeline));
...
bool VulkanRenderer::create_descriptor_set_and_pipeline_layout()
{
